  ...
  return (
    <section className="relative pt-32 pb-20 md:pt-40 md:pb-32 overflow-hidden bg-background selection:bg-primary/10">
      ...
      <div className="container mx-auto px-4 flex flex-col items-center text-center relative z-10">
        ...
        {/* --- 3D TILT DASHBOARD (LIGHT MODE FIX) --- */}
        <div
        ...
        >
          <motion.div
            ...
            className="relative rounded-xl border border-border/40 bg-white shadow-2xl overflow-hidden aspect-[16/10] md:aspect-[16/9] group"
          >
            {/* Fake Dashboard UI - LIGHT MODE */}
            <div className="absolute inset-0 bg-zinc-50 p-2 md:p-4">
              <div className="w-full h-full rounded-lg overflow-hidden bg-white border border-border/60 relative flex shadow-sm">
                {/* Sidebar */}
                <div className="w-16 md:w-60 border-r border-border/40 hidden md:flex flex-col p-4 gap-6 bg-zinc-50/50">
                  <div className="h-8 w-8 bg-primary rounded-lg mb-2 shadow-sm"></div>
                  <div className="space-y-3 opacity-60">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <div
                        key={i}
                        className="h-3 w-3/4 bg-zinc-200 rounded-full"
                      ></div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex-1 flex flex-col bg-white">
                  {/* Header */}
                  <div className="h-14 border-b border-border/40 flex items-center justify-between px-6 bg-white/80 backdrop-blur-sm">
                    <div className="h-4 w-32 bg-zinc-100 rounded-full"></div>
                  ...
                  <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6 overflow-hidden">
                    {/* Cards */}
                    <div className="h-32 rounded-xl border border-border/40 bg-white p-4 flex flex-col justify-between shadow-sm">
                      <div className="h-8 w-8 rounded-full bg-green-50 flex items-center justify-center text-green-600">
                        <TrendingUp className="h-4 w-4" />
                      </div>
                      <div className="h-6 w-24 bg-zinc-100 rounded-md"></div>
                    </div>
                    <div className="h-32 rounded-xl border border-border/40 bg-white p-4 flex flex-col justify-between shadow-sm">
                    ...
                    {/* Chart */}
                    <div className="md:col-span-2 h-64 rounded-xl border border-border/40 bg-white p-4 relative overflow-hidden shadow-sm group-hover:border-primary/20 transition-colors">
                    ...
                    {/* Recent List */}
                    <div className="h-64 rounded-xl border border-border/40 bg-white p-4 space-y-4 shadow-sm">
                      {[1, 2, 3, 4].map((i) => (
                        <div
                          key={i}
                        ...
                        >
                          <div className="flex gap-3">
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="h-2 w-10 bg-zinc-100 rounded-full"></div>
                            </div>
                          </div>
